...
class Logic:
    ...
    def evaluate(self, scope, options):
        ...
        if type(self.values_exp[0].result) is not BooleanBox:
            self.errors += [{"description": "Cannot use a non boolean operand in a logic operator", "id": self.id[0]}]
        ...
        for i, op in enumerate(self.ops_exp):
            ...
            # - Check if result is boolean
            if type(self.values_exp[i + 1].result) is not BooleanBox:
                self.errors += [{"description": "Cannot use a non boolean operand in a logic operator", "id": self.id[i]}]
            ...
            if op == '||':
                self.result = self.result.orOp(self.values_exp[i + 1].result)
            elif op == '&&':
                self.result = self.result.andOp(self.values_exp[i + 1].result)
